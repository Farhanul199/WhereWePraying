...
// (~200k rows) would blow the D1 free tier's 100,000-rows-written/day cap.
//
// STAGGERING STRATEGY: This endpoint processes all 553 mosques spread
// across ~30 daily runs (~19 mosques per day). The cron should run DAILY
// (recommended: 0 4 * * * for 04:00 UTC every day). Each day's batch
// is ~1,900 writes instead of all 50,000 at once. Combined with the
// WHERE clause in the UPSERT (only write if values changed), steady-state
// should be near zero writes most days, and well under quota even when
// data changes (DST, Ramadan, policy updates).
//
// Prayer times are fixed almost all year, and this rolling 90-day window
// refreshed daily always covers 2-3 months ahead.
//
...
//
// STAGGERING STRATEGY (to stay under D1 free tier, 100k writes/day):
// 
// This endpoint processes MYMASJID_MOSQUES in daily slices instead of all
// at once. It auto-calculates which slice to process based on the current
// day-of-month (1-31). ~553 mosques / ~30 days = ~19 per day, so each run
// is ~1,900 writes instead of ~50,000.
//
// For this to work, set the cron to run DAILY (not just Sundays):
//   RECOMMENDED CRON: 0 4 * * * (every day at 04:00 UTC)
// (current cron runs Sundays only — that's stale, change it after deploying this file)
//
// Manual testing: pass ?start=N&end=M to override and run a specific slice.
//   curl "https://wherewepraying.com/api/admin/sync-mymasjid-jamaah?start=0&end=19" \
//     -H "X-Sync-Key: YOUR_SYNC_SECRET"
...
function calculateDailySlice(dayOfMonth) {
  const MOSQUES_PER_DAY = 19; // 553 / 30 ≈ 18.4, round to 19
  const start = (dayOfMonth - 1) * MOSQUES_PER_DAY;
  const end = Math.min(start + MOSQUES_PER_DAY, MYMASJID_MOSQUES.length);
  return { start, end };
}
...
export async function onRequestGet(context) {
  ...
  if (startParam !== null && endParam !== null) {
    ...
    // Auto-stagger: use day-of-month to pick this day's slice
    // This spreads 553 mosques across ~30 days, ~19 per day
    const today = new Date();
    const dayOfMonth = today.getUTCDate();
    const daily = calculateDailySlice(dayOfMonth);
    start = daily.start;
    end = daily.end;
  }
